...
async function kv(command, ...args) {
  // FIX: Validasi env vars — tanpa ini fetch akan hit "undefined/..." dan silent fail
  if (!KV_URL || !KV_TOKEN) {
    throw new Error(
      "KV_REST_API_URL atau KV_REST_API_TOKEN tidak ditemukan. " +
      "Pastikan Vercel KV sudah di-connect ke project ini di dashboard Vercel."
    );
  }
  ...
  try {
    ...
    throw new Error(`KV fetch gagal (network error): ${err.message}`);
  }

  // FIX: Log response status agar mudah debug
  if (!res.ok) {
...
async function addToken(token) {
  ...
  // SADD mengembalikan jumlah item yang ditambahkan (0 = sudah ada, 1 = baru)
  console.log(`[STORAGE] addToken result: ${result} (0=sudah ada, 1=baru ditambah)`);
  return result;
}
